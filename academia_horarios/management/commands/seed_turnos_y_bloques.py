...
from academia_horarios.models import TurnoModel as Turno, Bloque
...
def cortar_en_bloques_de_40(inicio: time, fin: time) -> list[tuple[time, time]]:
    ...
    if total_min % BLOCK_MIN != 0:
        raise ValueError(f"El tramo {inicio}-{fin} no es múltiplo de {BLOCK_MIN} min (total={total_min}).")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        slug2turno = {}
        for code, slug, nombre, gkey in turnos_info:
