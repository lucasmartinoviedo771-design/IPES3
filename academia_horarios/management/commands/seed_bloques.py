...
from academia_horarios.models import TurnoModel, Bloque
    ...
    def handle(self, *args, **opts):
        ...
        for slug, cfg in config.items():
            ...
            for dia in [0,1,2,3,4,5]:  # lun..sab
                ...
                for i in range(cfg['bloques']):
                    es_recreo_actual = (orden in cfg['recreos'])
                    dur_actual = 20 if es_recreo_actual else 40
                    fin_actual = (datetime.combine(date.today(), h) + timedelta(minutes=dur_actual)).time()
                    
                    # Creamos el bloque principal (o el recreo si toca)
                    Bloque.objects.create(turno=turno, dia_semana=dia, orden=orden, inicio=h, fin=fin_actual, es_recreo=es_recreo_actual)
                    h = fin_actual
                    orden += 1
                    ...
                    if es_recreo_actual:
                        dur_clase = 40
                        fin_clase = (datetime.combine(date.today(), h) + timedelta(minutes=dur_clase)).time()
                        Bloque.objects.create(turno=turno, dia_semana=dia, orden=orden, inicio=h, fin=fin_clase, es_recreo=False)
                        h = fin_clase
                        orden += 1
